...
export default function Node({ project, selected, setSelected, query, hintActive }: Props) {
  const { size } = useThree()
  ...
  const basePos = useMemo(
    () => new Vector3(project.position[0], project.position[1], project.position[2]),
    [project.position]
  )
  ...
  useFrame((state, delta) => {
    ...
    if (selected && project.id !== selected.id && project.id !== 0) {
      selectedPos.set(selected.position[0], selected.position[1], selected.position[2])
    ...
    // 選択時ちょい拡大 + クリック反応
    const s =
      (isSelected ? 1.22 : 1.0) * nodeScale * (1 + clickPulseRef.current * 0.08)
    scaleVec.set(s, s, s)
  ...
  const labelStyle = useMemo(() => buildLabelStyle(project.color), [project.color])
  ...
  return (
    ...
    >
      {showLabel && (
        <Html position={[0, radius + 0.45, 0]} center style={{ pointerEvents: "none" }}>
      ...
      {hintActive && project.id === 0 && !selected && (
        <>
          ...
          <Html position={[0, radius + 0.9, 0]} center style={{ pointerEvents: "none" }}>
